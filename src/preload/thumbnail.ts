...
import { ThumbnailGenerationProgress, ThumbnailError, SavedSearch, SearchQuery } from '../shared/types/thumbnail';
...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // commands
  startThumbnailGeneration: (filePaths: string[]) => ipcRenderer.invoke('thumbnail-start', filePaths),
  ...
  // saved searches
  getSavedSearches: (): Promise<SavedSearch[]> => ipcRenderer.invoke('get-saved-searches'),
  ...
  updateSearchExecutionCount: (id: string) => ipcRenderer.invoke('update-search-execution-count', id),

  // テスト用: sync/async ping
